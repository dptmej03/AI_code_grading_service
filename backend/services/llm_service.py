...
async def grade_with_ai(
   ...
   - **부분점수 항목들** (0점 또는 해당 점수만 - 체크리스트 방식):
     ...
     - ⚠️ **루브릭 요구사항 정확히 읽기**: 항목에 "십진수로 표현"이라고 명시되어 있으면 이진수로 표현한 경우 틀린 것. 항목의 세부 요구사항을 반드시 확인 후 채점.
     - ⚠️ **독립 평가 원칙**: 다른 항목의 오류가 이 항목 점수를 결정하면 안 됨. 단, **이 항목 자체의 코드에 오류**(잘못된 함수 사용, 잘못된 인자, 잘못된 로직 등)가 있으면 0점.
     - ⚠️ **reason 필수 규칙**: reason은 반드시 해당 코드 로직에 대한 구체적 설명. '런타임 에러로 평가 불가' 절대 금지.
       - 맞은 경우 예: "cv2.add(img_gray, 80)으로 올바르게 밝기 조정 구현"
       - 틀린 경우 예: "np.bitwise_not은 인자 1개만 받는 함수인데 2개 전달하여 오류 발생", "루브릭에서 십진수 표현 요구했으나 이진수로 표현함"
...
4. **루브릭 요구사항 엄수** (⚠️ 매우 중요):
   - 각 항목의 요구사항을 **정확히** 읽고 판단. "십진수로 표현"이라고 했으면 이진수 표현은 틀린 것.
   - 결과값이 맞더라도 요구한 방식과 다르면 0점.

5. **에러와 채점의 독립성** (⚠️ 매우 중요):
   - reason에 '런타임 에러 발생으로 평가 불가', '실행되지 않아 평가 불가' 등 절대 금지
   - reason은 항상 해당 항목의 코드 로직에 대한 구체적 판단을 서술
   - **다른 항목의 오류**가 이 항목 점수를 결정하면 안 됨
   - **이 항목 자체의 코드에 오류**(잘못된 함수·인자·로직)가 있으면 → 0점 + 오류 이유 서술
