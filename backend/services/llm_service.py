...
4. **🎯 루브릭 요구사항 중심 평가** (⚠️ 가장 중요한 규칙):
   - reason은 반드시 **해당 루브릭 항목의 요구사항** 자체에 대한 평가여야 함
...
5. **에러와 채점의 독립성** (⚠️ CRITICAL 규칙):
   - 🚫 reason에 절대 금지 표현: '런타임 에러 발생으로 평가 불가', '실행되지 않아 평가 불가', '오류로 인해 생성 불가', '에러로 인해 평가 불가', '변수 미정의로 인해 에러' 등
   - ✅ reason은 항상 **해당 항목의 요구사항**에 대한 구체적 판단만 서술
   - **다른 항목의 오류**가 이 항목 점수를 결정하면 안 됨 (독립 평가 원칙)
   - **이 항목 자체의 코드에 로직 오류**(잘못된 함수·인자·로직)가 있으면 → 0점 + 로직 오류만 설명 (실행 오류 언급 금지)
   - **항목 요구사항과 무관한 코드 문제** (미정의 변수, 런타임 에러 가능성, 다른 셀의 오류 등)는 → reason이 아닌 **feedback의 개선점**에 작성
   - 실행 오류·코드 전반의 문제는 feedback 섹션에만 포함 (rubric_scores의 reason은 절대 금지)
